...
input_excel_filename = 'D:/Users/Kirker/Google Drive/Git Repositories/excel2latexviapython/Example/' +  \
                       'example_tables.xlsx'


# Select the directory/folder to save the resulting TeX files to
output_dir = 'D:/Users/Kirker/Google Drive/Git Repositories/excel2latexviapython/Example/'
...
for sheet_name in workbook.get_sheet_names():  # Loop over every worksheet/tab within the input workbook
    ...
    start_cell_label = list(sheet.rows)[start_row_idx][start_col_idx].column + str(list(sheet.rows)[start_row_idx][start_col_idx].row)
    end_cell_label = list(sheet.rows)[end_row_idx][end_col_idx].column + str(list(sheet.rows)[end_row_idx][end_col_idx].row)
    ...
    # Print to the terminal the name of the table file that is being created this iteration and the excel cells being
    # used to create it
    print('    ' + sheet_name + '.tex    ' + list(sheet.rows)[start_row_idx][start_col_idx].column + str(list(sheet.rows)[start_row_idx][start_col_idx].row) + ':'
    ...
    file.write('% Note: If your table contains colors, make sure \\usepackage[table]{xcolor} is included in the preamble \n')
    ...
    # For each row in the table's body create a string containing the tex code for that row and write to the output file
    for row_num in range(0, num_rows):
        ...
        hrule_str = e2l.create_horzrule_code(table_tuple[row_num], 'top', merge_start_cols, merge_end_cols,
                                             usr_settings, top_row=False, bottom_row=False)
