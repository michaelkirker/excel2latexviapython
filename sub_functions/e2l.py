...
def clean_cell_str(s):
    ...
    Sometimes (especially if the excel spreadsheet is generated by another program), cells
    are actually formated: [="CONTENTS"]. If this is the case, we want to return only the
    contents and not the =" at the start or " at the end.
...
def tupple2latexstring(row_tup, merge_list=[[], [], []]):
    """
    TUPPLE2LATEXSTRING
...
def round_num_in_str(str_in, num_dp):
    str_out = str_in
    # Extra a list of all numbers in the string
    ...
    for ii in range(0, len(list_found_num_str)):
        # For each number found, substute in the rounded number
